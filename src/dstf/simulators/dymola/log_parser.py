...
def _parse_translation_stats(text: str, stats: dict) -> None:
    ...
    if orig:
        ...
        _extract_int(
            orig_text, r"Variables:\s*(\d+)", stats, "translation", "original_variables"
        )
        _extract_int(
            orig_text,
            r"Unknowns:.*?(\d+)\s+scalars",
            stats,
            "translation",
            "original_unknowns_scalars",
        )
        _extract_int(
            orig_text,
            r"Differentiated variables:\s*(\d+)",
            stats,
            "translation",
            "differentiated_variables",
        )
        _extract_int(
            orig_text, r"Equations:\s*(\d+)", stats, "translation", "original_equations"
        )
        _extract_int(
            orig_text,
            r"Nontrivial:\s*(\d+)",
    ...
    if trans_match:
        ...
        init_split = re.split(
            r"^\s+Initialization problem\s*$", full_trans_text, flags=re.MULTILINE
        )
        ...
        _extract_int(
            sim_text,
            r"Continuous time states:\s*(\d+)",
            stats,
            "translation",
            "continuous_time_states",
        )
        _extract_int(
            sim_text,
            r"Time-varying variables:\s*(\d+)",
            stats,
            "translation",
        ...
        _extract_int(
            sim_text,
            r"Number of mixed real/discrete systems of equations:\s*(\d+)",
            stats,
            "translation",
            "mixed_systems",
        )
        _extract_int(
            sim_text,
            r"Number of numerical Jacobians:\s*(\d+)",
            stats,
            "translation",
            "numerical_jacobians",
        )
        ...
        if init_text:
            ...
            _extract_system_pair(
                ...
                stats,
                "translation",
                "init_homotopy_nonlinear",
            )

    # Continuous time states — fallback if not found in Translated Model section
    if "translation" not in stats or "continuous_time_states" not in stats.get(
        "translation", {}
    ):
    ...
    if states_match:
        state_names = [
            line.strip()
            for line in states_match.group(1).strip().split("\n")
            if line.strip()
        ]
        if state_names:
            stats.setdefault("translation", {})
...
def _extract_system_pair_simple(
    ...
    m = re.search(
        rf"Sizes of {sys_type} systems of equations\s*:\s*\{{([^}}]*)\}}", text
    )
    if m:
