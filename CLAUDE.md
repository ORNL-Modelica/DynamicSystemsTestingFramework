...
- **Tests**: refreshed six golden structural hashes (tube's cell content shrank; range's snapshot unchanged from my existing fixture's input-fill state). The panel-signature test now looks for `"See tube editor below plot"` on tube. Test count: 496 → 504.
