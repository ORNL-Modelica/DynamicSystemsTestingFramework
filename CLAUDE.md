...
**Phase 2 status (in progress; 2.1–2.3 complete)**: FMPy backend implemented as the second `SimulatorRunner`, exercising the Phase 1 capability abstraction. Reference-FMUs binaries fetched via `scripts/fetch_reference_fmus.py` (release-ZIP download, not submodule) into gitignored `examples/fmu/reference-fmus-binaries/`. `FmpyRunner` (`simulators/fmpy/runner.py`) simulates via `fmpy.simulate_fmu`, persists results to `<test_dir>/result.npz`, and reads back through the existing comparator contract. `test_spec.json` gained an optional `"fmu"` field; `BatchManifest.mat_file` (dead Dymola-specific code) removed from backend-agnostic base. **Remaining: 2.4** — end-to-end via CLI against `examples/fmu/testing.json` + committed baselines for a Reference-FMU subset. **2.5** — GitHub Actions CI example proving the FMPy path runs without Dymola. Decisions D48–D50 in [docs/decisions.md](docs/decisions.md).
