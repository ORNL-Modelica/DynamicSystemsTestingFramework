...
class TestSchemaParsing:
    ...
    def test_unknown_match_type_raises(self):
        with pytest.raises(
            RecognizerSpecError, match="unknown match type 'inheritance'"
        ):
            parse_recognizer_spec(
                {
                    "name": "x",
    ...
    def test_parameter_source_with_extends_match_raises(self):
        ...
        with pytest.raises(
            RecognizerSpecError, match="isn't compatible with match type 'extends'"
        ):
            parse_recognizer_spec(
                {
                    "name": "x",
    ...
    def test_parameter_source_requires_name(self):
        with pytest.raises(
            RecognizerSpecError, match=r"\(source=parameter\) requires 'name'"
        ):
            parse_recognizer_spec(
                {
                    "name": "x",
    ...
    def test_constant_source_requires_value(self):
        with pytest.raises(
            RecognizerSpecError, match=r"\(source=constant\) requires 'value'"
        ):
            parse_recognizer_spec(
                {
                    "name": "x",
...
class TestClassNameGlob:
    ...
    def test_glob_does_not_match_other(self, tmp_path):
        ...
        rec = parse_recognizer_spec(
            {
                "name": "test:power",
                "match": {"type": "class-name-glob", "class_pattern": "MyLib.Tests.*"},
            }
        )
        assert rec.recognize(mo) is None
    ...
    def test_class_name_glob_requires_pattern(self):
        with pytest.raises(
            RecognizerSpecError, match="class-name-glob requires 'class_pattern'"
        ):
            parse_recognizer_spec(
                {
                    "name": "x",
...
class TestAnnotationFieldSource:
    ...
    def test_annotation_source_requires_annotation_key(self):
        with pytest.raises(
            RecognizerSpecError, match=r"\(source=annotation\) requires 'annotation'"
        ):
            parse_recognizer_spec(
                {
                    "name": "x",
...
class TestExperimentAnnotationField:
    ...
    def test_paths_include_filters(self, tmp_path):
        ...
        for sub, name in [("Examples", "Wanted"), ("Internal", "Skipped")]:
            (tmp_path / sub / f"{name}.mo").write_text(
                textwrap.dedent(f"""
                within MyLib.{sub};
                model {name}
                  extends Modelica.Icons.Example;
