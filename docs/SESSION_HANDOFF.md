...
**TRANSFORM-on-Dymola-Linux re-characterization (this session, HEAD `0d4dad9`)**: 326-test suite via persistent worker, parallel=1, 60s/test timeout. Result: **242 PASS (74.2%) + 84 license-tier translation failures + 0 timeouts + 0 worker-broken + 0 restart-exhausted events**. The 84 failures all carry the actual Dymola error `"the model is too complex for the current license. Your license must be upgraded to handle this model."` in their per-test `translation_log.txt` files (verified) — not framework artifacts, not cascade replays. Run wall time ≈ 20 min for sim + ~94s for report generation. Framework is rock-solid on this WSL→server setup; the 84 license-tier failures are deterministic Dymola license behavior on this machine's licensed feature set.

**Diagnosis correction from prior session**: the previous handoff claimed the 84 license-tier failures were "broken-worker cascade replaying stale cached error log content, not actual license issues." **That diagnosis was incorrect.** Verification against the raw per-test `translation_log.txt` files confirms the messages are real Dymola license errors, not stale cache replays. With proper worker recovery (D90 in place — reset-on-success counter + translation-log health probe), the 84 still legitimately fail at the license check. Lesson: when a classifier identifies a failure mode, verify against the raw artifact (the actual log file content) rather than inferring from secondary signals (process behavior, cascade patterns).

**User-reported "worker broken / dead / restart exhausted" symptoms not reproduced**: The user reported these on Linux Dymola earlier this session. Not reproduced with parallel=1 single-worker config. Most plausible reproducer not yet tested: `--parallel N>1` where multiple workers contend for Dymola license slots simultaneously. Documented as an open issue likely environmental to the user's WSL→server setup (per user's prompt: "if it is a server/license issue in the end we do our best and then just leave it as a future task/open issue").
...
| **`Dymola/linux/` TRANSFORM baselines partial** | 320 baseline files exist; 84 tests legitimately fail at translation due to Dymola license-tier limits on this machine. Not a framework issue; the licensed feature set on this Dymola install does not include the features needed for those 84 models (mostly large multiphysics test cases — Pumps, Heat Exchangers, NSSS, IRIS reactor model, etc.). | Skip-list those 84 tests in test_spec.json with `"simulate_only": false` + a comment, OR upgrade the Dymola license tier. Either is a workplace decision, not a framework fix. |
| **`--parallel N>1` Linux Dymola broken-worker cascade** (suspected) | User reported worker-broken / dead / restart-exhausted events on Linux Dymola earlier this session. Not reproduced this session with parallel=1. Most plausible cause: license contention when multiple workers spawn simultaneously — license server queues some, the slow ones look like dead workers to the framework. | (a) test with `--parallel 2/4` against TRANSFORM and capture per-worker logs, (b) consider staggering worker startup with a `time.sleep` between spawns to space out license requests, (c) accept as environmental for this WSL→server setup |
...
* **Investigate Linux Dymola timeouts.** ✅ Characterized this session at HEAD `0d4dad9`. TRANSFORM 326-test run via persistent worker, parallel=1: 242 PASS / 84 license-tier-FAIL / 0 timeouts / 0 worker-broken / 0 restart-exhausted. The 84 failures are genuine Dymola license-tier errors visible in per-test `translation_log.txt` files. Framework is healthy. Prior session's "97% pass with 7 problem tests" diagnosis (3 timeouts + 4 collateral) was based on a different license configuration AND a misreading of the failure mode (the "84 license-tier" was attributed to stale-log replay; actually genuine). User-reported "worker broken / dead / restart exhausted" was not reproduced with parallel=1; possibly a `--parallel N>1` license-contention scenario, marked as a future open issue per the user's "if it's a server/license issue, leave it" guidance.
