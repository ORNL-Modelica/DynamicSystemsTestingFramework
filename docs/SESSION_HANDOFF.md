...
**Date**: 2026-04-30
**Covers**: five arcs over one session — modelica-testing CLI sweep + CLI subcommand parity (`1780121`–`ac7417b`); annotation ↔ test_spec.json contract docs (`b0124ad`); dashboard + report unification (`b6053e1`–`2a8f7da`, 11 commits); dashboard UX polish from two rounds of feedback (`aede963`–`334b060`, 7 commits); **Linux Dymola timeout cascade root-cause + fix** (`2a7e338` + `2c5f1d3`).
**State at HEAD** (commit `2c5f1d3`):
...
- **TRANSFORM-on-Dymola-Linux 97.85% pass rate** (319 / 326), down from 7% (23 / 326 cascade-failed) before the worker_killed fix.
...
> Resuming DSTF (Dynamic Systems Testing Framework) at commit `2c5f1d3` on `main`. The previous session was a 5-arc pass: v1.0-cleanliness, dashboard unification, 2 rounds of dashboard polish, **and a Linux-Dymola persistent-worker cascade fix** that took TRANSFORM-on-Linux from a 7% pass rate (cascade-failing) to 97.85%:
>
