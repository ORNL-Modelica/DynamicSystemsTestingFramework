...
* **Unify dashboard + report into one progressively-enriching page** (Option 2, ~2 days + ½ day JS-fetch upgrade). Today there are two HTML surfaces — `dashboard.html` (live, inline f-string template in `progress.py`) and `reports/index.html` (post-run, Jinja). They drift, and the live page is missing sort/filter while the post-run page is missing live elapsed/auto-refresh. **Direction**: one Jinja template + shared JS feeds a single `dashboard.html` that progressively enriches as data arrives. While running: status/worker/elapsed populated; NRMSE/warnings/translate/sim columns dashed. After comparison: same page rewritten with those columns filled in + per-test deep-dive links, auto-refresh stripped. `--report` flag narrows to controlling only per-test `interactive.html` generation; the unified dashboard always exists. Replaces ~80 lines of duplicated f-string template in `progress.py`. **Filter/sort**: status-button filter (port from index.html) + per-column text filter (new). 3-state sort cycle (no-sort → sorted → reverse → no-sort) with **desc-first on numeric columns** (largest NRMSE first when triaging) and asc-first on text columns. **Refresh**: switch `<meta http-equiv="refresh">` to `setInterval(fetch('status.json'))` + DOM update so scroll position survives; pair with discoverable "Refresh now" button.

* **Audit + replace stale `modelica-testing` CLI references** (~1 hr). User-facing strings in source: `src/dstf/reporting/plot_comparison.py:1183/1193/1195` (rerun-command builder still emits `modelica-testing --config ... run`), `src/dstf/reporting/templates/interactive.html:354` (`spec-update` instruction copy), `src/dstf/reporting/templates/interactive.js:3761` (badge tooltip). Plus internal cache path `src/dstf/simulators/dymola/interface_loader.py:127` (`~/.cache/modelica-testing/dymola-interface/` → `~/.cache/dstf/...`; one-time ~2s cache miss for existing devs, no functional impact). **Out of scope**: `docs/decisions.md` historical entries (audit value), stale generated artifacts under `reports/`, `testing_output/` (auto-regenerate next run).

* **Tighten the annotation ↔ test_spec.json contract** (~1.5 days). Grilling outcome: do **NOT** extend `Components.UnitTests` with simulation parameters (StopTime / Algorithm / etc.) and do **NOT** add per-field booleans or `null`-as-sentinel for "use annotation." That would create a three-layer puzzle (annotation OR UnitTests OR spec) and couple library source to test-tool semantics — the existing two-layer system already handles "editor simulates one way, tests simulate another" via `experiment(...)` annotation + per-test `simulation.*` override. **Keep the contract** (already in code, just under-documented): user omits → annotation if present, else simulator default; user provides → used. **Tighten it** with: (1) extend `src/dstf/discovery/mo_parser.py:_parse_experiment` to read `StartTime` and `Interval`/`NumberOfIntervals` (today only `StopTime` + `Tolerance` parsed); (2) document the two-layer contract in CLAUDE.md + `docs/usage.md`; (3) expand example `test_spec.json` files to show the under-used fields (`start_time`, `number_of_intervals`, `output_interval`) — schema already supports them; (4) add per-test `algorithm` / `solver` override in test_spec.json (currently config-only via `simulator_setup`); (5) add a **resolution-explainer** — either a column in the unified dashboard or a `--explain` flag — showing provenance per field (e.g. `stop_time: 10 (annotation)` vs `stop_time: 10 (test_spec)`). This solves the real "where did this value come from?" pain without inventing new layers.
