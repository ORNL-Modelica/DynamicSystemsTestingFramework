...
| Backend | `simulators/` registry with two concrete backends: `DymolaRunner` (native Modelica via Dymola Python interface / batch `.mos`) and `FmpyRunner` (FMU via FMPy Python API). Both declare `capabilities` + `produced_datasets`. | Registry + capabilities contract validated across two backends with distinct transport models. Cross-backend verification (chaining Dymola FMU-export → FMPy) is a future composition. |
...
| Metric | `comparison/modes.py`: `NrmseMode`, `TubeMode`, `FinalOnlyMode`, `RangeMode`; `VariableComparison` carries `diagnostics: dict` for structured extras | strategy pattern in place; `RangeMode` proves the contract isn't NRMSE-shaped (doesn't consume the reference trajectory) |
| MetricTree | `comparison/metric_tree.py` (combinators + `MetricResult`), `comparison/tree_spec.py` (parsed user spec), `comparison/tree_eval.py` (spec → evaluated tree). `compare_test` derives `passed` from the tree root; users author trees via `test_spec.json` `"metrics"` block | wired end-to-end Phase 3; implicit flat-AND remains the default when no spec is authored |
